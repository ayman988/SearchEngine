...
public class BTreeTest<T extends Comparable<T>> {
    ...
    public ArrayList<T> traverse() {
        Queue<Node<T>> BFSQue = new LinkedList<>();
        BFSQue.add(root);
        ArrayList<T> keys = new ArrayList<>();
        while (BFSQue.size() > 0) {
            Node<T> curNode = BFSQue.poll();
            if (curNode == null) {
                return keys;
            }
            for (int i = 0; i < curNode.keysSize; i++)
                keys.add(curNode.keys[i]);
            for (int i = 0; i < curNode.childrenSize;i++)
                BFSQue.add(curNode.children[i]);
        }
        return keys;
    }
    ...
    private static class TreePrinter {
        ...
        public static <T extends Comparable<T>> String getString(BTreeTest<T> tree) {
    ...
    public static class JavaCompatibleBTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
        ...
        private BTreeTest<T> tree = null;

        public JavaCompatibleBTree(BTreeTest<T> tree) {
            this.tree = tree;
        }
        ...
        private static class BTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            private BTreeTest<C> tree = null;
            private BTreeTest.Node<C> lastNode = null;
            private C lastValue = null;
            private int index = 0;
            private Deque<BTreeTest.Node<C>> toVisit = new ArrayDeque<BTreeTest.Node<C>>();

            protected BTreeIterator(BTreeTest<C> tree) {
                this.tree = tree;
            ...
            public C next() {
                ...
                while (toVisit.size() > 0) {
                    ...
                    BTreeTest.Node<C> n = toVisit.pop();
